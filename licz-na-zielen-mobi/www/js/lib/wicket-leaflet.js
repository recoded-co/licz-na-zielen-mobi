...
L.MultiPolyline||(obj.constructor===L.MultiPolygon||(obj.constructor===L.LayerGroup||obj.constructor===L.FeatureGroup))){features=[];tmp=obj._layers;for(attr in tmp)if(tmp.hasOwnProperty(attr))if(tmp[attr].getLatLngs||tmp[attr].getLatLng)features.push(this.deconstruct(tmp[attr]));return{type:function(){switch(obj.constructor){case L.MultiPolyline:return"multilinestring";case L.MultiPolygon:return"multipolygon";case L.FeatureGroup:return function(){var i,mpgon,mpline,mpoint;mpgon=true;mpline=true;
mpoint=true;for(i in obj._layers)if(obj._layers.hasOwnProperty(i)){if(obj._layers[i].constructor!==L.Marker)mpoint=false;if(obj._layers[i].constructor!==L.Polyline)mpline=false;if(obj._layers[i].constructor!==L.Polygon)mpgon=false}if(mpoint)return"multipoint";if(mpline)return"multilinestring";if(mpgon)return"multipolygon";return"geometrycollection"}();default:return"geometrycollection"}}(),components:function(){var i,comps;comps=[];for(i=0;i<features.length;i+=1)if(features[i].components)comps.push(features[i].components);
